...
def process_images(queue_embeddings, queue_images, output_directory, logger, stop_event):
    """
    Processes images from the queue: performs face detection and recognition, then saves processed images,
    and creates a results table.
    """
    ...
    while not stop_event.is_set():
        try:
            ...
            if not queue_images.empty():
                ...
                # Detect faces in the image to get the total number of faces (face detection)
                total_faces_bboxes, _ = detector.detect(img, var.max_num)
                total_faces = len(total_faces_bboxes)  # Total faces detected in the image

                # Process image and collect results
                detected_faces = []  # List to store detection results
                recognized_faces = 0  # Counter for recognized faces
                try:
                    ...
                    if processed_img is not None:
                        ...
                        # Count recognized faces (face recognition)
                        ...
                        if detected_faces:
                            ...
                            summary_data = {
                                ...
                                "Total Faces Recognized": [recognized_faces],
                                "Detection Rate (%)": [recognition_rate],  # Detection rate from total_faces detected
                        ...
                        logger.info(f"Total faces detected: {total_faces}, Recognized faces: {recognized_faces}, Detection rate: {recognition_rate:.2f}%, Average recognition score: {avg_recognition_score:.2f}")
                        print(f"Total faces detected: {total_faces}, Recognized faces: {recognized_faces}, Detection rate: {recognition_rate:.2f}%, Average recognition score: {avg_recognition_score:.2f}")
                except Exception as e:
