...
log_obj = Logger_Days("mongodb_logging")
...
def log_to_mongo(best_match_name: str, current_datetime: datetime, collection, name_camera):
    ...
    try:
        new_log = {
            "WriteDate": datetime.now(timezone.utc),
            "SN": str(name_camera),
            "Pin": best_match_name,
...
def log_to_mongo_if_not_in_json(best_match_name: str, current_datetime: datetime, collection, name_camera, path_logs, queue_faces):
    global log_obj  # Sử dụng biến toàn cục
    try:
        ...
        if best_match_name not in data:
            ...
            queue_faces.put([best_match_name, formatted_time, name_camera])
